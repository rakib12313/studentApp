...
const App: React.FC = () => {
    ...
    const startExam = (exam: Exam) => {
        ...
        if (!user) return;
        if (state.results.some(r => r.examId === exam.id && r.studentId === user.uid)) {
        ...
        if(confirm(`Start ${exam.title}? This will enter full-screen mode.`)) {
            setActiveExam(exam);
    ...
    return (
        <div className="min-h-screen pb-24 md:pb-0 md:pl-24 text-white font-sans selection:bg-blue-500/30">
            ...
            <main className="p-6 md:p-12 max-w-7xl mx-auto animate-fade-in">
                ...
                {activeTab === 'results' && <ResultsTab results={state.results} studentId={user.uid} exams={state.exams} />}
...
const ResultsTab = ({ results, studentId, exams }: { results: ExamResult[], studentId: string, exams: Exam[] }) => {
